...
from oslo_config import cfg#该配置可以直接写配置文件，节点【】。属性=
...
def register_opt_group(conf, opt_group, options):#注册配置选项
    if opt_group:#如果存在这个配置选项表
        conf.register_group(opt_group)#直接利用conf的包进行注册组
    for opt in options:
        conf.register_opt(opt, group=getattr(opt_group, 'name', None))#将每一个配置项都注册

'''
以下的配置类似。只是将不同的专用字符串改成了简单的类似字典结构。
大部分都有help属性
有的可以初始化为空。
'''
auth_group = cfg.OptGroup(name='auth',
                          title="Options for authentication and credentials")#此项为验证，opensatck的admin等的环境配置
...
#注册配置选项
def register_opts():#读完配置文件，进行此步操作
    ext_plugins = plugins.TempestTestPluginManager()#初始化插件
    # Register in-tree tempest config options
    for g, o in _opts:
        register_opt_group(_CONF, g, o)#将上面每一个配置模式加入到注册选项组中
    # Call external plugin config option registration
    ext_plugins.register_plugin_opts(_CONF)#每个配置传到下层去注册
...
def list_opts():
    ...
    opt_list = [(getattr(g, 'name', None), o) for g, o in _opts]#getattr()。参数是一个对象,一个字符串和一个任意值。字符串可能会列出一个现有的属性或一个新的属性
    opt_list.extend(ext_plugins.get_plugin_options_list())# extend()方法只接受一个列表作为参数，并将该参数的每个元素都添加到原有的列表中。
    return opt_list
...
class TempestConfigPrivate(object):
    ...
    DEFAULT_CONFIG_DIR = os.path.join(os.getcwd(), "etc")#os.getcwd()该函数不需要传递参数，它返回当前的目录

    DEFAULT_CONFIG_FILE = "tempest.conf"

    def __getattr__(self, attr):#获取属性
        # Handles config options from the default group
        return getattr(_CONF, attr)

    def _set_attrs(self):#建立属性，与之前的对应，发现带_feature_的都是bool类型
    ...
    def __init__(self, parse_conf=True, config_path=None):#初始化配置文件
        """Initialize a configuration from a conf directory and conf file."""
        super(TempestConfigPrivate, self).__init__()#？？父进程有初始化？？哪里的父进程
        config_files = []
        failsafe_path = "/etc/tempest/" + self.DEFAULT_CONFIG_FILE#失败安全文件目录

        if config_path:#配置文件的目录可以自己获取
            path = config_path
        else:#环境变量覆盖默认值
        ...
        if not os.path.isfile(path):#默认文件路径没找到，在失败安全文件目录路径找
            path = failsafe_path
        ...
        logging.register_options(_CONF)#这个logging文件，变成了oslo的模板log文件
        if os.path.isfile(path):#如果找到这个路径，并且是一个文件
            _CONF([], project='tempest', default_config_files=config_files)#基本的配置就是这个文件
        else:
            _CONF([], project='tempest')
        logging.setup(_CONF, 'tempest')
        LOG = logging.getLogger('tempest')
        LOG.info("Using tempest config file %s" % path)
        register_opts()#运行配置选项
        self._set_attrs()#将配置文件，写入
        if parse_conf:
            _CONF.log_opt_values(LOG, std_logging.DEBUG)


class TempestConfigProxy(object):#log有关配置
    _config = None
    _path = None
    ...
    def _fix_log_levels(self):#调整配置log级别
    ...
    def __getattr__(self, attr):
        if not self._config:
            ...
            self._config = TempestConfigPrivate(config_path=self._path)#生成log文件，建立类
